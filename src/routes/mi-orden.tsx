...
import { ShoppingCart, Trash2, Plus, Minus, Search, Save, X, ExternalLink, Sparkles } from "lucide-react";
...
export const Route = createFileRoute("/mi-orden")({
  component: MiOrdenPage,
  head: () => ({
    meta: [
...
function MiOrdenPage() {
  ...
  // Mix más barato: por cada item, farmacia con menor precio
  const mixPlan = useMemo(() => {
    let total = 0; let totalMax = 0; const rows: { item: OrderItem; pharmId: string | null; ves: number | null; price: PriceRow | null; max: number | null }[] = [];
  ...
  // Por farmacia: total por farmacia que tiene cobertura, ordenadas por total
  const pharmPlans = useMemo(() => {
    return pharms.map((ph) => {
      let total = 0; let covered = 0; const detail: { item: OrderItem; ves: number | null; price: PriceRow | null }[] = [];
      for (const it of items) {
    ...
    .sort((a, b) => {
      // Cobertura completa primero, luego menor total
  ...
  return (
    <div className="container mx-auto px-4 py-6 md:py-10 pb-24 md:pb-10 space-y-6">
      <header className="space-y-2">
        <h1 className="text-2xl md:text-3xl font-bold flex items-center gap-2">
          <ShoppingCart className="h-6 w-6 text-primary" /> Crea tu orden de medicinas
        </h1>
        <p className="text-sm text-muted-foreground">
          Arma tu lista, te decimos dónde comprarla más barata y cuánto ahorras frente al precio más caro.
        </p>
      </header>
        ...
        <>
          <Card className="p-4 space-y-3">
            <div className="flex items-center justify-between gap-2">
              <h2 className="font-semibold">Tu lista ({items.length})</h2>
          ...
          <Card className="p-4 space-y-3">
            <h2 className="font-semibold flex items-center gap-2"><Save className="h-4 w-4" /> Guardar mi orden</h2>
            <div className="flex gap-2">
  ...
  return (
    <Card className="p-4 space-y-4">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h2 className="font-semibold">Compra más barata combinando farmacias</h2>
